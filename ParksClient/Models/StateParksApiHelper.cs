...
namespace ParksClient.Models
{
    class StateParksApiHelper
    {
        ...
        {
            RestClient client = new RestClient("http://localhost:5001/api");
            RestRequest request = new RestRequest($"stateparksv2/{id}", Method.GET);
            var response = await client.ExecuteTaskAsync(request);
            return response.Content;
        }
        ...
        {
            RestClient client = new RestClient("http://localhost:5001/api");
            RestRequest request = new RestRequest($"stateparksv2", Method.POST);
        ...
        {
            RestClient client = new RestClient("http://localhost:5001/api");
            RestRequest request = new RestRequest($"stateparksv2/{id}", Method.PUT);
            request.AddHeader("Content-Type", "application/json");
            request.AddJsonBody(newStatePark);
            var response = await client.ExecuteTaskAsync(request);
        }
        ...
        {
            RestClient client = new RestClient("http://localhost:5001/api");
            RestRequest request = new RestRequest($"stateparksv2/{id}", Method.DELETE);
            request.AddHeader("Content-Type", "application/json");
            var response = await client.ExecuteTaskAsync(request);
        }
    }
